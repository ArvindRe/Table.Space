...
col VALUE            for a20
col PQ_STATUS        for a12      head "STATUS"
...
SELECT s.inst_id            AS qc_inst,
       ...
       s.program,
       ps.degree,
       (SELECT COUNT(*) FROM gv\$px_session p2
        WHERE  p2.qcinst_id = s.inst_id AND p2.qcsid = s.sid) AS pq_sessions
FROM   gv\$px_session ps
JOIN   gv\$session s ON s.inst_id = ps.inst_id AND s.sid = ps.qcsid
WHERE  ps.qcsid = ps.sid          -- QC row only
${INST_WHERE_B}
ORDER  BY pq_sessions DESC;

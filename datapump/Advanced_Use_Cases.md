...
This document maps specific DBA workflows to the existing script suite.
...
> **`datapump.sh` best practices are applied automatically.** All parfiles generated by `datapump.sh` include `metrics=Y`, `logtime=ALL`, `compression=ALL/MEDIUM`, `cluster=N`, and `exclude=STATISTICS` for data exports. All data impdp parfiles include `metrics=Y`, `logtime=ALL`, `exclude=GRANT,REF_CONSTRAINT,TRIGGER,INDEX,CONSTRAINT`, and `PRE/POST-IMPORT` SQL comment blocks. See the [Scripts Usage Guide](Datapump_Scripts_Guide.md) for the full parameter reference.
...
Restore system settings, validate, then re-apply deferred DDL. The export dumpfile contains full metadata (indexes, constraints, triggers, grants) — the `exclude=` was only on the import side — so re-applying is a dumpfile read with `content=METADATA_ONLY`.
...
**Step 4 — Re-apply DDL via `_post` parfile (standard approach)**

If the import was generated by `datapump.sh`, the `_post.par` parfile is already created. For manual imports (as in this use case), create a parfile with `content=METADATA_ONLY` and `include=INDEX,CONSTRAINT,REF_CONSTRAINT,TRIGGER,GRANT`, then run:
...
impdp db_user/$$$$$$$$@TARGET_PDB \
    dumpfile=expdp_LOB_TABLE_0_%U.dmp \
    directory=DATA_PUMP_DIR \
    logfile=impdp_LOB_TABLE_post.log \
    content=METADATA_ONLY \
...
**Step 4 — Alternative: SQLFILE extraction (for per-index parallel DDL)**

If individual indexes are large and you need each `CREATE INDEX` to use parallel query slaves independently, extract the DDL to a SQL file instead of executing it directly:

```
# Extract DDL to file — does not execute, writes to the Oracle DIRECTORY path
job_name=impdp_LOB_TABLE_ddl
tables=SCHEMA.LOB_TABLE
directory=DATA_PUMP_DIR
dumpfile=expdp_LOB_TABLE_0_%U.dmp
logfile=impdp_LOB_TABLE_ddl.log
sqlfile=post_import_LOB_TABLE.sql
content=METADATA_ONLY
include=INDEX,CONSTRAINT,REF_CONSTRAINT,TRIGGER,GRANT
```

Then apply with parallel DDL in SQL*Plus:

```sql
ALTER SESSION FORCE PARALLEL DDL PARALLEL 32;
@<oracle_directory_path>/post_import_LOB_TABLE.sql
ALTER SESSION DISABLE PARALLEL DDL;
```

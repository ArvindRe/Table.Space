...
**Location:** Scripts are in the `datapump` folder on the target server.
...
```sql
CREATE DIRECTORY "DATA_PUMP_DIR1" AS '/nfs/datapump/export/';
```

SQL scripts for DIRECTORY creation are kept alongside the parfiles. Verify the OS path is writable by the `oracle` OS user before running the export.

---

### `query=` Parameter Formatting

When a `query=` parameter contains single quotes (e.g. in `TO_DATE()` calls), escape them with backslashes — the parfile parser interprets `\'` as a literal single quote before passing the SQL to Oracle:

```
# Correct — backslash-escaped single quotes
query=SCHEMA.TABLE:"WHERE col >= to_date(\'01-MAY-25\',\'DD-MON-YY\')"

# Incorrect — will fail with ORA-00911 or a parsing error
query=SCHEMA.TABLE:"WHERE col >= to_date('01-MAY-25','DD-MON-YY')"
```

Rules:
- Prefix the value with `SCHEMA.TABLE:` followed by the WHERE clause in **double quotes**
- Include the `WHERE` keyword inside the double quotes
- Tables exported as full loads simply omit the `query=` parameter entirely

```
query=SCHEMA.TABLE:"WHERE col1 IN (...) AND col2 IN (...)"
```

...
extract_last_dump_block() {
    ...
    END {
        ...
        if (last_header > 0) {
            ...
            for (j=last_header; j<=NR; j++) {
                ...
                if (l ~ /^[[:space:]]+/ || l ~ /\.dmp(\.|$)/) {
                    ...
                    if (l ~ /^[[:space:]]*Job (Master table|Log file|Legacy|Total elapsed)/ || l ~ /ORA-/ || l ~ /EXP-/ || l ~ /UDE-/ || l ~ /IMP-/ || l ~ /UDI-/) break
                } else {
        ...
        # 3) Last resort: print the last contiguous block of lines containing .dmp tokens
